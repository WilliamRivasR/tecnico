...
class RegistrationFrame(tk.Frame):
    ...
    def create_widgets(self):
        ...
        ttk.Button(self.scrollable_frame, text="Register", command=self.register).grid(column=0, row=len(fields)+1, columnspan=2, pady=20)
        ttk.Button(self.scrollable_frame, text="Back to Login", command=self.show_login).grid(column=0, row=len(fields)+2, columnspan=2)

        self.message = ttk.Label(self.scrollable_frame, text="")
        self.message.grid(column=0, row=len(fields)+3, columnspan=2, pady=10)
